...
func GetAnalysisLegacy(c *fiber.Ctx) error {
	db := c.Locals("db").(*gorm.DB)

	buildingID := c.Params("building_id")

	var analysis database.Analysis
	result := db.Select("external_building_id AS building_id, neighborhood_id, construction_year, construction_year_reliability, foundation_type, foundation_type_reliability, restoration_costs, drystand, drystand_risk, drystand_risk_reliability, bio_infection_risk, bio_infection_risk_reliability, dewatering_depth, dewatering_depth_risk, dewatering_depth_risk_reliability, unclassified_risk, height, velocity, ground_water_level, ground_level, soil, surface_area, owner").
		First(&analysis, "external_building_id = ?", buildingID)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"message": "Analysis not found"})
	...
	return c.JSON(fiber.Map{
		"buildingId":                  analysis.BuildingID,
		"neighborhoodId":              analysis.NeighborhoodID,
		"constructionYear":            analysis.ConstructionYear,
		"constructionYearReliability": analysis.ConstructionYearReliability,
		"recoveryType":                analysis.RecoveryType,
		"restorationCosts":            analysis.RestorationCosts,
		"height":                      analysis.Height,
		"velocity":                    analysis.Velocity,
		"groundWaterLevel":            analysis.GroundWaterLevel,
		"groundLevel":                 analysis.GroundLevel,
		"soil":                        analysis.Soil,
		"surfaceArea":                 analysis.SurfaceArea,
		"damageCause":                 analysis.DamageCause,
		"enforcementTerm":             analysis.EnforcementTerm,
		"overallQuality":              analysis.OverallQuality,
		"inquiryType":                 analysis.InquiryType,
		"foundationType":              analysis.FoundationType,
		"foundationTypeReliability":   analysis.FoundationTypeReliability,
		"drystand":                    analysis.Drystand,
		"drystandReliability":         analysis.DrystandReliability,
		"drystandRisk":                analysis.DrystandRisk,
		"dewateringDepth":             analysis.DewateringDepth,
		"dewateringDepthReliability":  analysis.DewateringDepthReliability,
		"dewateringDepthRisk":         analysis.DewateringDepthRisk,
		"bioInfectionReliability":     analysis.BioInfectionReliability,
		"bioInfectionRisk":            analysis.BioInfectionRisk,
		"unclassifiedRisk":            analysis.UnclassifiedRisk,
	})
}
