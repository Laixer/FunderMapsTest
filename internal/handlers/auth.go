...
func SigninWithPassword(c *fiber.Ctx) error {
	...
	db.Transaction(func(tx *gorm.DB) error {
		...
		tx.Exec("INSERT INTO application.auth_session (user_id, ip_address, application_id, provider, updated_at) VALUES (?, ?, ?, 'jwt', now()) ON CONFLICT ON constraint auth_session_pkey DO UPDATE SET updated_at = excluded.updated_at, ip_address = excluded.ip_address;", user.ID, c.IP(), "app-0blu4s39")
